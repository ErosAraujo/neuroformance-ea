...
type HomeErrors = Partial<
  Record<
    "record" | "summary" | "recovery" | "alerts" | "history" | "indicators",
    string
  >
>;
...
function formatTrend(value?: string | null) {
  ...
  if (["melhorando", "melhora", "subindo"].includes(normalized)) return "Subindo";
  if (["piorando", "queda", "caindo", "descendo"].includes(normalized)) return "Descendo";
  if (["estavel", "estável"].includes(normalized)) return "Estável";
...
export default function HomeScreen() {
  ...
  const [selectedIndicatorKey, setSelectedIndicatorKey] =
  ...
  const fetchHomeData = useCallback(async () => {
    ...
    if (recordRes.status === "fulfilled") setLastRecord(recordRes.value.data);
      ...
      nextErrors.record = getApiErrorMessage(
        recordRes.reason,
        "Não foi possível carregar o último registro.",
      );

    if (summaryRes.status === "fulfilled")
      setWeeklySummary(summaryRes.value.data);
    else
      nextErrors.summary = getApiErrorMessage(
        summaryRes.reason,
        "Não foi possível carregar o resumo semanal.",
      );

    if (recoveryRes.status === "fulfilled")
      setRecoverySummary(recoveryRes.value.data);
    else
      nextErrors.recovery = getApiErrorMessage(
        recoveryRes.reason,
    ...
    if (alertsRes.status === "fulfilled")
      ...
      nextErrors.alerts = getApiErrorMessage(
        alertsRes.reason,
    ...
    if (historyRes.status === "fulfilled")
      ...
      nextErrors.history = getApiErrorMessage(
        historyRes.reason,
    ...
    if (indicatorsRes.status === "fulfilled") {
      ...
      nextErrors.indicators = getApiErrorMessage(
        indicatorsRes.reason,
  ...
  const avatarInitials = useMemo(() => initialsFromStudentName(user?.name), [user?.name]);
  ...
  const confidenceText =
    confidence !== null ? `Confiança: ${confidence}%` : "Confiança: em cálculo";
  const recordsBaseText = indicators?.recordsUsed
    ? `Base: últimas ${indicators.recordsUsed} noite${indicators.recordsUsed > 1 ? "s" : ""}`
    : lastRecord
      ? "Base: último registro disponível"
      : "Base: aguardando registros";
  const statusGeneral = useMemo(
    () => getStatusGeneralScore(indicators),
    [indicators],
  );
  ...
  const statusRecommendation =
    getStatusGeneralRecommendation(statusClassification);
  const hasErrors = Object.keys(errors).length > 0;
  const weeklyAverageHours = formatAverageHours(weeklySummary?.averageHours);
  const selectedDefinition =
    metricDefinitions.find((metric) => metric.key === selectedIndicatorKey) ||
    null;
  const selectedData = selectedDefinition
    ? getMetric(indicators, selectedDefinition.key)
    : null;
  const selectedColor = selectedDefinition
    ? getIndicatorColor(
        selectedData?.classification,
        selectedDefinition.negative,
        selectedData?.value,
        selectedDefinition.key,
      )
    : colors.muted;
  const weeklyStatusPoints = useMemo(
    () => buildWeeklyStatusPoints(historyRecords, statusGeneral.score),
    [historyRecords, statusGeneral.score],
  );
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
          ...
          <View style={styles.brandBlock}>
            ...
            <View style={styles.brandTextBlock}>
              ...
              <Text style={styles.brandSubtitle}>
                Sistema de prontidão, reflexo e performance
              </Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.greetingBlock}>
          <View style={styles.greetingRow}>
            <View style={styles.greetingTextBlock}>
              ...
              <Text style={styles.greetingName} numberOfLines={2} adjustsFontSizeToFit>
                {user?.name || "Aluno"}!
              </Text>
              <Text style={styles.greetingText}>
                Seu corpo fala. Nós traduzimos.
              </Text>
            </View>
            <View style={styles.studentAvatar}>
              {avatarSource && !avatarFailed ? (
                ...
                <Text style={styles.studentAvatarText}>{avatarInitials}</Text>
              )}
            </View>
          </View>
        </View>
        ...
        {selectedDefinition ? (
          ...
          <View
            style={[
              ...
              {
                borderColor:
                  statusColor === colors.muted ? colors.border : statusColor,
              },
            ]}
          >
            <View style={styles.mainCardContent}>
              <View style={styles.mainRingBlock}>
                <ReadinessRing
                  value={statusGeneral.score}
                  color={
                    statusColor === colors.muted ? colors.primary : statusColor
                  }
                  size={118}
                  strokeWidth={12}
                />
              ...
              <View style={styles.mainInfo}>
                ...
                <Text style={[styles.mainStatus, { color: statusColor }]}>
                  {normalizeClassification(statusClassification).toUpperCase()}
                </Text>
                <Text style={styles.mainDescription}>
                  Resumo geral do seu estado físico e mental hoje, com base nos
                  sinais de recuperação, energia, foco, fadiga e risco de
                  sobrecarga.
                </Text>
              </View>
            </View>
            ...
            {statusGeneral.componentsCount > 0 && (
              <Text style={styles.componentsText}>
                Cálculo feito com {statusGeneral.componentsCount} {statusGeneral.componentsCount > 1 ? "indicadores disponíveis" : "indicador disponível"}.
              </Text>
            )}
            {baseReduced && (
              <Text style={styles.reducedBaseText}>
                Base reduzida: cálculo feito com menos de 3 noites registradas.
              </Text>
            )}
          </View>
        )}
        ...
        <Pressable
          ...
          onPress={() =>
            navigation.navigate("Registrar", { resetFormNonce: Date.now() })
          }
        >
          <MaterialCommunityIcons
            name="weather-night"
        ...
        {errors.history && (
          <Text style={styles.inlineWarning}>{errors.history}</Text>
        )}
        ...
        {weeklySummary ? (
          <>
            ...
            <View style={styles.summaryGrid}>
              <SummaryCard
                value={Number(weeklySummary.averageScore).toFixed(1)}
                label="Média"
              />
              <SummaryCard
                value={formatTrend(weeklySummary.trend)}
                label="Tendência"
                accent={colors.secondary}
              />
              <SummaryCard
                value={String(weeklySummary.goodNights)}
                label="Boas noites"
              />
              {weeklyAverageHours && (
                <SummaryCard
                  value={weeklyAverageHours}
                  label="Média de sono"
                  accent={colors.primary}
                />
              )}
              {typeof weeklySummary.adherence === "number" && (
                <SummaryCard
                  value={`${Number(weeklySummary.adherence).toFixed(0)}%`}
                  label="Adesão"
                />
              )}
              {typeof weeklySummary.badNights === "number" && (
                <SummaryCard
                  value={String(weeklySummary.badNights)}
                  label="Noite ruim"
                />
              )}
            </View>
          ...
          <View style={styles.alertHeader}>
            <View style={styles.alertIcon}>
              <MaterialCommunityIcons
                name="shield-check-outline"
                size={21}
                color={alerts.length ? colors.warning : colors.success}
              />
            </View>
            <View style={styles.alertBody}>
              <Text style={styles.alertTitle}>
                {alerts.length ? "Atenção aos alertas" : "Tudo sob controle"}
              </Text>
              {errors.alerts ? (
...
function SummaryCard({ value, label, accent = colors.text }: SummaryCardProps) {
  ...
  return (
    <View style={styles.summaryCard}>
      <Text
        style={[styles.summaryValue, compact && styles.summaryValueCompact, { color: accent }]}
        numberOfLines={2}
